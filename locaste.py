...
class LocasteSpider(SSBaseSpider):
    ...
    def start_requests(self):
        for country_code, language_code, currency, url in self.countries:
            ...
            if country_code is 'jp':
                yield Request(url, callback=self.parse_homepage, meta=meta)
            else:
                yield Request(url, callback=self.tr_parse_homepage, meta=meta)

    def tr_parse_homepage(self, response):
    ...
    def make_request(self, response, url, catogaries):
        ...
        if meta['country_code'] is not 'jp':
            return Request(response.urljoin(url), callback=self.tr_parse_product, meta=meta)
        return Request(response.urljoin(url), callback=self.parse_product, meta=meta)

    def tr_parse_product(self, response):
        ...
        for product in response.css('.product-item-box'):
            ...
            yield Request(item['url'], callback=self.tr_parse_detail, meta={'item': item})
    ...
    def tr_parse_pagination(self, response):
        pages = response.css('.pagination a::text').extract()
        for page in pages:
            url = '?page={}'.format(page)
            return Request(response.urljoin(url), callback=self.tr_parse_product, meta=response.meta)

    def tr_parse_detail(self, response):
        ...
        for color_id in color_ids:
            ...
            yield Request(url, callback=self.tr_parse_color, meta={'item': item})

    def tr_parse_color(self, response):
        ...
        color_item['size_infos'] = self.tr_get_size_info(response, color_item)
        print(color_item)

    def tr_get_size_info(self, response, color_item):
        size_info = []
